...
// Process each empty ingredient array
const lines = content.split('\n');
let updatedContent = content;
let currentItem = null;
let currentCategory = null;

// First pass: identify items with empty arrays
for (let i = 0; i < lines.length; i++) {
  ...
  } else if (currentItem && line.includes('name: "')) {
    currentItem.name = line.match(/name: "([^"]+)"/)?.[1];
  } else if (currentItem && line.includes('description: "')) {
    currentItem.description = line.match(/description: "([^"]+)"/)?.[1];
  } else if (currentItem && line.includes('category: "')) {
    currentItem.category = line.match(/category: "([^"]+)"/)?.[1];
  }
  
  // Check for empty ingredients array
  if (currentItem && line.trim() === 'ingredients: [],') {
    const { ingredients, allergens, dietary } = getItemData(
      currentItem.name || '',
    ...
    const ingredientsStr = ingredients.length > 0 
      ...
      : '';
    const allergensStr = allergens.length > 0
    ...
    const oldPattern = 'ingredients: [],\\n    allergens: [],\\n    dietary: [],';
    const newPattern = `ingredients: [${ingredientsStr}],\\n    allergens: [${allergensStr}],\\n    dietary: [${dietaryStr}],`;
    
    updatedContent = updatedContent.replace(
      new RegExp(oldPattern.replace(/\[/g, '\\[').replace(/\]/g, '\\]')),
      newPattern
    );
  }
}
...
console.log('Menu ingredients updated successfully!');